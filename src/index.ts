...
function languageFromMarker(raw: string): string | undefined {
  const segments = raw.toLowerCase().split('/').map(s => s.trim()).filter(Boolean);
  if (segments.length === 0) return undefined;
  if (!segments.every(s => /^[a-z]+$/.test(s))) return undefined;
...
export function parseOimlPubid(src: string, bibdataYear = ''): OimlPubid | null {
  ...
  for (;;) {
    ...
    if (peek()?.kind === 'word' && peek()!.value.toLowerCase() === 'edition' && t[i + 1]?.kind === 'num') {
      eat();
      const v = eat()!.value;
      if (v.length === 4) year ??= v; else edition ??= v;
      continue;
    }
    break;
  }
...
export function urnForIdentifier(src: string, bibdataYear = ''): string | null {
